...
- **Recurring transactions:** `useRecurringInstances` generates virtual `Transaction` objects from `RecurringTransaction` rules for the selected month. Dashboard calculations stay scoped to the loaded selected-month transaction rows, while calendar/reminder surfaces continue consuming `recurringInstances` for forward-looking recurring visibility.
...
- **ViewTransitions implementation:** Implemented the first phase of the ViewTransition analysis plan by establishing shared element transition on the "Expenses by Category" card in the Dashboard summary (`ExpensesByCategorySummary`), which flows seamlessly into the detailed "Category Analysis" view (`CategoryAnalysis`/`ExpensesByCategoryChart`) on navigation. The transition uses `<ViewTransition name="expenses-by-category-title">` for the header/title block and `<ViewTransition name="expenses-by-category-chart">` for the interactive pie chart container.
...
10. **Statistics category-limit semantics:** analytics now use `/transactions/all`, but the statistics category-limit cards still read `category.monthlyData[currentMonth]` for current-month limits/spend. That mixed historical/current-month behavior remains intentional unless the product contract changes.
